...
const Home: React.FC = () => {
  ...
  return (
    <div className={`relative flex flex-col min-h-screen ${isDarkMode ? 'bg-gradient-to-br from-gray-900 via-gray-800 to-black animate-gradient-dark' : 'bg-gradient-to-br from-blue-100 via-blue-300 to-indigo-200 animate-gradient-light'} transition-colors duration-500 font-poppins overflow-hidden`}>
      <div className="absolute inset-0 z-0 opacity-20">
        <svg className="w-full h-48 sm:h-64 md:h-80 lg:h-full animate-slide-in" viewBox="0 0 1440 320" xmlns="http://www.w3.org/2000/svg">
          <path
      ...
      <div className="relative z-20">
        <img
          ...
          className="absolute top-2 sm:top-4 md:top-6 right-2 sm:right-4 md:right-6 w-12 sm:w-16 md:w-20 lg:w-24 rounded-full shadow-lg transition-all duration-300 animate-pulse-glow hover:filter hover:drop-shadow-[0_0_12px_rgba(59,130,246,0.8)] dark:hover:drop-shadow-[0_0_12px_rgba(96,165,250,0.8)]"
        />
      </div>

      <div className="relative z-10 text-center mt-1 sm:mt-2 text-black dark:text-white text-xs sm:text-sm md:text-base drop-shadow-lg font-bold animate-fade-in">
        {currentTime.toLocaleDateString()} | {currentTime.toLocaleTimeString()}
      </div>

      <div className="relative z-10 flex flex-col items-center justify-center pt-4 sm:pt-8 md:pt-12 pb-2 sm:pb-4 md:pb-6">
        <div className="flex flex-col md:flex-row items-center justify-center gap-2 sm:gap-4 md:gap-8 w-full max-w-4xl mx-auto">
          <h1
            key={currentHeadline}
            className="relative text-lg sm:text-xl md:text-2xl lg:text-3xl xl:text-4xl font-extrabold text-center dark:text-white bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent animate-slide-up-fade max-w-[90%] sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl mx-auto drop-shadow-xl"
          >
        ...
        <p className="font-semibold mt-1 sm:mt-2 text-sm sm:text-base md:text-lg text-white dark:text-white animate-fade-in max-w-[90%] sm:max-w-sm mx-auto drop-shadow-xl">
          Plan your day with precision weather data.
        </p>
      </div>

      <div className="relative z-10 flex-1 flex items-center justify-center p-2 sm:p-4 md:p-6">
        <div className="w-full max-w-xl sm:max-w-2xl bg-white/90 dark:bg-gray-800/90 backdrop-blur-lg rounded-2xl sm:rounded-3xl shadow-lg p-3 sm:p-4 md:p-6 transform transition-all duration-500 hover:shadow-xl">
          <div className="flex flex-col sm:flex-row justify-between items-center mb-2 sm:mb-4 gap-2 sm:gap-4">
            <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 dark:text-white drop-shadow-2xl">Weather App</h2>
            <button
              onClick={toggleDarkMode}
              className="px-3 sm:px-4 py-1 sm:py-2 bg-gray-200 dark:bg-gray-700 rounded-full shadow-md hover:shadow-lg transition-all duration-300 transform hover:scale-105"
            >
              <span className={`block w-8 sm:w-10 h-4 sm:h-6 bg-gray-400 rounded-full p-0.5 sm:p-1 ${isDarkMode ? 'bg-blue-600' : 'bg-gray-400'} relative`}>
                <span className={`absolute w-3 sm:w-4 h-3 sm:h-4 bg-white rounded-full transition-transform duration-300 ${isDarkMode ? 'translate-x-4 sm:translate-x-6' : 'translate-x-0'}`}></span>
              </span>
            </button>
          </div>

          <div className="relative group z-10">
            <div className="absolute -inset-1 sm:-inset-1.5 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-xl sm:rounded-2xl blur opacity-25 group-hover:opacity-75 transition-all duration-500 group-hover:-inset-2 sm:group-hover:-inset-2.5"></div>
            <div className="relative rounded-lg sm:rounded-xl py-1 px-2 sm:py-1.5 sm:px-2.5 transition-all duration-300">
              <SearchBar />
            </div>
          </div>
          ...
          {history.length > 0 && (
            <div className="mt-3 sm:mt-4 animate-fade-in">
              <h3 className="text-base sm:text-lg font-semibold text-gray-700 dark:text-white mb-1 sm:mb-2">Recent Searches:</h3>
              <div className="flex flex-wrap gap-1 sm:gap-2">
                {history.map((city) => (
                  <button
                    key={city}
                    onClick={() => dispatch(fetchWeather(city))}
                    className="bg-blue-500 text-white px-2 sm:px-3 py-1 sm:py-1.5 rounded-full text-xs sm:text-sm font-medium hover:bg-blue-700 transition-all duration-200 transform hover:-translate-y-0.5 hover:shadow-md"
                  >
                    {city}
                  </button>
                ))}
          ...
          {loading && (
            <p className="text-gray-700 dark:text-white text-center mt-3 sm:mt-4 animate-pulse">Loading...</p>
          )}
          ...
          {data && (
            <div className="mt-3 sm:mt-4 animate-scale-fade">
      ...
      <div className="relative z-10 text-center py-1 sm:py-2">
        <p className="text-sm sm:text-base md:text-lg text-black dark:text-white animate-fade-in mb-1 sm:mb-2 drop-shadow-lg">
